...
if battle_state = BATTLE_STATE.PLAYER{
	if battle_buttom_state = MENU.BUTTOM_CHOICE{
		if not instance_exists(battle_ui_dialogue) {
			...
			battle_ui_dialogue = CreateLangText(52,270,DEPTH.UI_TOP,GetLangStruct("*" + battle_ui_dialogue_text.text),battle_ui_dialogue_color);
		}
	}else{
...
if battle_state = BATTLE_STATE.ENCOUNTER_TEXT {
	...
	if not instance_exists(battle_ui_dialogue) {
		if Battle_Dialogue_Size() > 0 {
			battle_ui_dialogue = CreateLangText(52,270,DEPTH.UI_TOP,"* " + Battle_Dialogue_Get());
