...
if step_func != undefined {
    ...
    if time <= cost_time {
    	...
    	rot = default_rot + (target_rot-default_rot) * animcurve_channel_evaluate(animcurve_get_channel(ac_bone_rot,0),t);
    }else if time >= cost_time and time - cost_time <= keep_time  {
    	rot += rotspeed;
    	bone_long = bone_max_long;
    	x += lengthdir_x(move_speed,movedir);
    	y += lengthdir_y(move_speed,movedir);
    }else if time - cost_time >= keep_time {
    	...
    	rot = default_rot + (target_rot-default_rot) * animcurve_channel_evaluate(animcurve_get_channel(ac_bone_rot,0),1-t);
    	if t >= 1 {
    		instance_destroy();
    	}
    }
