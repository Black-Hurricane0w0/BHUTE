...
var summon_char = function(){
    var hy = string_height(string_char_at(text,char_index)) * font_get_size(text_font) / 18 * text_size + 4;
    ...
    char_x += string_width(string_char_at(text,char_index)) * font_get_size(text_font) / 18 * text_size//加x
	char_index ++;
	if inst.text == " " {
		ttime = text_speed;
        char_x += 6;
